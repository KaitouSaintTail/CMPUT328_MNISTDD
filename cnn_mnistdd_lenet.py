...
def Conv(inputs, kernel_size, strides, num_filters, weight_decay, name):
    ...
    with tf.variable_scope("conv2D_%s" % name):
        ...
        conv = tf.nn.conv2d(inputs, w, strides=[1, strides, strides, 1], padding="VALID")
...
class CNN():
    def __init__(self, num_classes=10, num_digits=2, weight_decay=5e-4):
        ...
        # CNN Architecture
        self.conv1 = Conv(inputs=self.input_x_reshaped, kernel_size=5, strides=1, num_filters=6, weight_decay=weight_decay, name="conv1")
        self.pool1 = tf.nn.max_pool(self.conv1, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='VALID', name="pool1")

        self.conv2 = Conv(inputs=self.pool1, kernel_size=5, strides=1, num_filters=16, weight_decay=weight_decay, name="conv2")
        ...
        self.flatten = tf.reshape(self.pool2, (-1, shape))

        self.fc1 = fc(inputs=self.flatten, dropout_keep_prob=self.dropout_keep_prob, num_outputs=128, name='fc1')
        self.fc2 = fc(inputs=self.fc1, dropout_keep_prob=self.dropout_keep_prob, num_outputs=84, name='fc2')

        with tf.variable_scope('fc'):
            w = tf.get_variable('w', [self.fc2.get_shape()[1], num_classes*num_digits], initializer=kernel_initializer)
